...
class Logger {
    ...
    log(level, logId, filename, func, message, cls=null, args=null) {
        ...
        if (config.LOG_LEVEL === 0 && level === "debug") {
            const logMessage = this.prepare_log("DEBUG", logId, filename, func, message, cls, args)
            console.debug(logMessage)
            return
        }

        if (config.LOG_LEVEL < 2 && level === "info") {
            const logMessage = this.prepare_log("INFO", logId, filename, func, message, cls, args)
            console.info(logMessage)
            return
        }

        if (config.LOG_LEVEL < 3 && level === "warning") {
            const logMessage = this.prepare_log("WARNING", logId, filename, func, message, cls, args)
            console.warn(logMessage)
            return
        }
        
        if (config.LOG_LEVEL < 4 && level === "error") {
            const logMessage = this.prepare_log("ERROR", logId, filename, func, message, cls, args)
    ...
    prepare_log(logLevel, logId, filename, func, message, cls, args) {
